...
def middleware_function(function):
    async def wrapper(message):
        current_state = await get_state(message.chat.id)
...
async def function_message(message, function, *args):
    response_text = await function(message.chat.id, *args)
...
async def handler_start(message):
    ...
    await function_message(message, register_user, message.chat.first_name)
